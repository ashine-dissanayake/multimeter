...
void ILI9163_init(int rotation) {
    ...
    ILI9163_writeData(
        0x07); // NLA = 1, NLB = 1, NLC = 1 (all on Frame Inversion)

    /* This bit may cause minor compatibility issues, so it's commented out.
    Beware. ILI9163_writeCommand(ILI9163_CMD_POWER_CONTROL1);
    ...
    ILI9163_writeData(0x02); // BT = 2: AVDD = 2xVCI1, VCL = -1xVCI1, VGH =
    5xVCI1, VGL = -2xVCI1
    ...
    ILI9163_writeData(0x40); // nVM = 0, VMF = 64: VCOMH output = VMH, VCOML
    output = VML
    */
...
void ILI9163_render() {
    ...
    SPI_DMA_FL = 0;
    while (!SPI_DMA_FL) {
    } // This can be commented out if your thread sends new frames slower than
      // SPI transmits them. Otherwise, memory havoc. See README.md
}
